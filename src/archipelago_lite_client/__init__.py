...
class GameConfig:
    ...
    def __init__(
        self,
        game: str,
        items_handling: ItemsHandlingFlags = ItemsHandlingFlags.ReceiveItems,
        hash: str = "",
    ):
        self.game = game
...
class Client:
    ...
    async def _packet_sender(self):
        ...
        """
        while (
            self._active or not self._packages_to_be_sent.empty()
        ) and self._connection.state is State.OPEN:
            if self._handshake_done and not self._packages_to_be_sent.empty():
                packages = [
                    self._packages_to_be_sent.get()
                    for _ in range(min(self._packages_to_be_sent.qsize(), 10))
                ]
                try:
            ...
            try:
                ...
                for packet in packets:
                    match packet:
                        ...
                        case Connected():
                            logging.info(
                                "Connection established to the archipelago server"
                            )
                        ...
                        case ConnectionRefused():
                            logging.exception(
                                "The connection was refused, may be due to wrong client info"
                            )
            ...
            asyncio.create_task(
                self._send_packages(
                    Connect(
                        ...
                        self.client_config.player,
                        self.client_config.client,
                        self.client_config.version,
                        self.game_config.items_handling,
                        [],
                        True,
                    )
                )
    ...
    async def run(self):
        ...
        with self._state_lock:
            if self._active or (
                self._connection and self._connection.state is State.OPEN
            ):
        ...
        while self._active:
            try:
                self._connection = await connect(
                    f"{'wss' if self.client_config.ssl else 'ws'}://{self.client_config.address}:{self.client_config.port}"
                )
                counter = 0
                ...
                if self._active:
                    logging.error(
                        "The connection to the archipelago server was closed unexpectedly, will try to reconnect"
                    )
